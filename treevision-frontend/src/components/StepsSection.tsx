...
const steps = [
  {
    ...
    icon: <MdImageSearch size={34} className="text-[#2394C8]" />,
  },
  {
    step: 2,
    ...
    <section className="bg-white rounded-lg shadow-lg p-6 flex flex-col gap-4 w-full">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 px-4 py-12">
        {steps.map((item) => (
          <div
            key={item.step}
            className="relative bg-[rgba(163,214,241,0.30)] border border-gray-200 rounded-3xl p-8 shadow-md hover:shadow-2xl transition-all duration-300 group"
          >
            {/* Círculo con número */}
            <div className="absolute -top-5 left-1/2 -translate-x-1/2">
              <div className="w-12 h-12 bg-[#2394C8] text-white flex items-center justify-center text-xl font-bold rounded-full shadow-lg ring-4 ring-white group-hover:scale-110 transition-transform duration-300">
                {item.step}
              </div>
            </div>
            ...
            <div className="mt-3 text-center px-2">
              <h3 className="text-xl font-semibold text-gray-900 mb-3 border-b-2 border-[#2394C8] inline-block pb-1">
                {item.title}
              </h3>
